...
export async function generateStaticParams() {
  return getSolutionLandingParams();
}
...
export async function generateMetadata({ params }: RouteProps): Promise<Metadata> {
  ...
  const landing = await getSolutionLanding(localeParam, slug);
  if (!landing) {
    return {};
  }
...
export default async function SolutionLandingPage({ params }: RouteProps) {
  const { locale: localeParam, slug } = await params;
  ...
  const landing = await getSolutionLanding(locale, slug);
    ...
    },
    {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          item: absoluteUrl(`/${locale}/solutions`),
        },
        {
          "@type": "ListItem",
          position: 3,
          name: landing.title,
          item: absoluteUrl(`/${locale}/solutions/${landing.slug}`),
        },
      ],
    },
  ];
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <CardModule>
              <h2 className="font-display text-2xl text-text-primary">
                {locale === "he" ? "מה תקבלו בפועל" : "What You Will Get"}
              </h2>
